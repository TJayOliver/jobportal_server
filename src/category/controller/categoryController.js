class CategoryController {
  ...
  async createCategory(req, res) {
    try {
      ...
      if (category.error)
        return res.status(409).json({ message: category.error });
      return res
        .status(201)
        .json({ message: "Successfully Created", data: category });
    } catch (error) {
  ...
  async readAllCategory(req, res) {
    try {
      ...
      return res
        .status(201)
        .json({ message: "Successfully Retrieved", data: category });
    } catch (error) {
  ...
  async updateCategory(req, res) {
    try {
      ...
      const category = await this.service.updateCategoryService(
        categoryDetails
      );
      return res
        .status(201)
        .json({ message: "Successfully Updated", data: category });
    } catch (error) {
  ...
  async deleteCategory(req, res) {
    try {
      ...
      return res
        .status(201)
        .json({ message: "Successfully Deleted", data: category });
    } catch (error) {
